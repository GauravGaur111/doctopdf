...
Jruby gem to convert microsoft office documents to pdf. Useful to render office documents in browser in rails web app settings. Uses openoffice and jodcoverter libraries to convert to pdf. 
 ...
 Usage: Doctopdf.convert_to_pdf(input_file, output_path), where 
        input_file is the input file with complete path and 
        output_path is the path wehre output pdf will be produced.
        output_path defaults to the current directory.
 Returns: converted pdf file is places in the output directory. 
